...
class CartController extends Controller
{
    ...
    public function apply_coupon_cart(Request $request) { DB::enableQueryLog();
        $coupon_code = $request->coupon_code;
        if(isset($coupon_code)) {
            $coupon = Coupon::where('code', $coupon_code)->where('expiry_date', '>=', Carbon::today()->format('Y-m-d'))
                ->where('cart_value', '<=', str_replace(',', '', Cart::instance('cart')->subtotal()))
                ->first();
//dd(DB::getQueryLog());
            if(!$coupon) {
    ...
    public function calculateDiscount() {
        ...
        if(Session::has('coupon')) {
            ...
            $textAfterDiscount = $subtotalAfterDiscount * config('cart.tax') / 100;
            $totalAfterDiscount = $subtotalAfterDiscount + $textAfterDiscount;

            Session::put('discounts', [
                'discount' => number_format(floatval($discount), 2, '.', ''),
                'subtotal' => number_format(floatval($subtotalAfterDiscount), 2, '.', ''),
                'tax' => number_format(floatval($textAfterDiscount), 2, '.', ''),
